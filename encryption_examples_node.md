...
Here we can find an example of how to encrypt / decrypt the identification data on NodeJS
...
export class PgpUtils {
    ...
    static async generateKeys(): Promise<openpgp.SerializedKeyPair<string>> {
        return await openpgp.generateKey({
            type: "ecc",
            curve: "curve25519",
            userIDs: { name: "IPSP API", comment: "Auto generated from the IPSP API" }
        });
    }

    static async encrypt(message: string, pgpPublicKeys: string[]): Promise<string> {
    ...
    static async decrypt(armoredMessage: string, pgpPrivateKey: string): Promise<openpgp.DecryptMessageResult> {
...
import { PgpUtils } from "../utils/pgp/pgpUtils";
import { v4 } from "uuid";

export async function encryptCredentials(): Promise<void> {
    const keys = await PgpUtils.generateKeys();
    ...
    const encryptedEncryptionSecret = await PgpUtils.encrypt(
        encryptionSecret, 
        encryptKeysList
    );
